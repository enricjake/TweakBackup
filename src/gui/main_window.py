...
This module implements the primary graphical user interface for WinSet, providing:
- A tabbed interface (Home, Presets, Manual Configuration, History)
- Expandable setting rows with sliders, buttons, and text entry controls
- Preset creation and application wizards
- Settings import/export functionality
- System restore point integration
- Direct links to Windows Settings pages

The UI uses a dark-mode Fluent Design-inspired theme with the Tkinter 'clam' style.
"""

import subprocess  # Used to launch system tools (control.exe, ms-settings URIs, etc.)
import tkinter as tk  # Core GUI toolkit
from tkinter import ttk, messagebox, filedialog  # Themed widgets, dialogs, file pickers
import sys  # System-level operations (e.g., sys.executable for restart)
...
# ── Manager / backend imports ────────────────────────────────────────────────
# Each manager handles a specific domain of WinSet functionality.
from src.storage.exporter import ProfileExporter  # Serialize settings to JSON files
from src.storage.importer import ProfileImporter  # Load settings from JSON files
from src.presets.preset_manager import (
    PresetManager,
)  # Load/apply built-in & custom presets
from src.utils.backup_manager import BackupManager  # System restore point creation
from src.core.registry_handler import (
    RegistryHandler,
)  # Read/write Windows Registry values
from src.core.powershell_handler import (
    PowerShellHandler,
)  # Execute PowerShell commands safely
from src.core.setting_loader import (
    SettingLoader,
)  # Parse settings.json into model objects
from src.core.history_manager import HistoryManager  # Log changes to SQLite database
...
class MainWindow:
    ...
    Orchestrates all UI tabs (Home, Presets, Manual, History) and delegates
    system operations to the various manager/handler backend classes.

    Attributes:
        root: The root Tkinter window instance.
        bg_color / fg_color / accent_color / border_color:
            Dark-mode colour palette used throughout the UI.
        notebook: The ttk.Notebook that hosts the four main tabs.
        *_frame: Individual tab frames (home, presets, manual, history).
        *_canvas / *_scrollable / *_window_id:
            Canvas-based scrolling infrastructure for each tab.
        manual_vars: Maps setting IDs to their Tkinter control variables.
        expanded_setting_id: Tracks which setting row is currently expanded.
        is_busy: Flag preventing concurrent background operations.
        restart_pending: Set True when a change requires a system restart.
        preset_manager / importer / exporter / backup_manager /
        registry_handler / powershell_handler / setting_loader /
        history_manager: Backend managers for each domain.
    """

    def __init__(self, root: tk.Tk):
        """Initialize the MainWindow, build the UI, and centre it on screen.

        Args:
            root: The root Tk instance created by winset.py main().
        """
        from src.version import VERSION  # Lazy import to avoid circular deps

        self.root = root
        self.root.title(f"WinSet - v{VERSION} - Windows Configuration Toolkit")

        # ── Theme colours (set properly in _setup_ui) ────────────────────
        self.bg_color: str = ""
        self.fg_color: str = ""
        self.accent_color: str = ""
        self.border_color: str = ""

        # ── Widget references (created in _setup_ui / tab builders) ──────
        ...
        # Manual Configuration tab scrolling infrastructure
        self.manual_canvas: tk.Canvas
        self.manual_scrollable: tk.Frame
        self.manual_window_id: int
        self.manual_vars: Dict[str, Any] = {}
        self.expanded_setting_id: Optional[str] = None

        # Home tab scrolling infrastructure
        self.home_canvas: tk.Canvas
        self.home_scrollable: tk.Frame
        self.home_window_id: int

        # Presets tab scrolling infrastructure
        self.presets_canvas: tk.Canvas
        self.presets_scrollable: tk.Frame
        self.presets_window_id: int
        ...
        # ── Backend managers (one per domain) ────────────────────────────
        ...
        # Build the full UI and centre the window on screen
        self._setup_ui()
        self.center_window()
    ...
    def center_window(self):
        ...
        Forces Tk to process pending geometry changes first (update_idletasks)
        so that winfo_width/height return the actual rendered size, then
        calculates the x/y offset to centre the window on screen.
        """
    ...
    def select_all_categories(self):
        """Check every category checkbox (used by export/import dialogs)."""
        for var in self.category_vars.values():
            var.set(True)

    def clear_all_categories(self):
        """Uncheck every category checkbox (used by export/import dialogs)."""
        for var in self.category_vars.values():
            var.set(False)

    def _setup_ui(self):
        """Build the full application UI: theme, notebook, and all four tabs.

        Steps:
        1. Select the Tkinter 'clam' theme (falls back to 'default' on error).
        2. Define a dark-mode colour palette inspired by Windows 11 Fluent Design.
        3. Configure ttk styles for every widget type (buttons, labels, tabs, etc.).
        4. Create the main notebook with four tabs: Home, Presets, Manual, History.
        5. Delegate to per-tab builder methods (_create_*_tab).
        """
        # ── Theme selection ──────────────────────────────────────────────
        style = ttk.Style()
        try:
            style.theme_use("clam")  # Best cross-platform dark-mode support
        except Exception:
            try:
                style.theme_use("default")  # Fallback if 'clam' unavailable
            except Exception:
                pass  # Last resort: keep whatever theme is already active

        # ── Dark-mode colour palette (Windows 11 Fluent Design inspired) ─
        ...
        border_color = "#1e2130"  # Subtle border/divider
        tab_bg = "#1a1d2e"  # Darker tab background (unselected)
        tab_selected = "#0f1117"  # Match main background when selected

        # Store colours as instance attributes for use in other methods
        ...
        style.map(
            "Accent.TButton", background=[("active", "#0056b3"), ("pressed", "#004085")]
        )
        ...
        # Notebook widget provides the tabbed interface
        self.notebook = ttk.Notebook(self.main_frame)
        self.notebook.pack(fill=tk.BOTH, expand=True)

        # Create tab frames (content is built by the _create_*_tab methods)
        self.home_frame = ttk.Frame(self.notebook)
        self.presets_frame = ttk.Frame(self.notebook)
        self.manual_frame = ttk.Frame(self.notebook)
        self.history_frame = ttk.Frame(self.notebook)

        # Register tabs with the notebook
        ...
        # Fire _on_tab_changed whenever the user switches tabs
        # (used to refresh content and rebind scroll events)
    ...
    def _create_home_tab(self):
        """Build the Home tab: welcome banner, quick actions, and system tools.

        Layout:
        - Welcome section with app name and description.
        - Quick Actions: export, import, create restore point.
        - System Tools: shortcuts to Control Panel, Services, MSConfig, etc.
        All content is placed inside a scrollable canvas.
        """
        ...
        self.home_canvas = tk.Canvas(
            scroll_frame, highlightthickness=0, bg=self.bg_color
        )
        ...
        self.home_scrollable.bind(
            "<Configure>",
            lambda e: self.home_canvas.configure(
                scrollregion=self.home_canvas.bbox("all")
            ),
        )
        self.home_window_id = self.home_canvas.create_window(
            (0, 0), window=self.home_scrollable, anchor="nw"
        )
        self.home_canvas.configure(yscrollcommand=home_scroll.set)

        self.home_canvas.bind(
            "<Configure>",
            lambda e: self.home_canvas.itemconfigure(
                self.home_window_id, width=e.width
            ),
        )
        ...
        actions_frame = ttk.LabelFrame(
            self.home_scrollable, text="Quick Actions", padding=20
        )
        ...
        tools_frame = ttk.LabelFrame(
            self.home_scrollable, text="System Tools", padding=20
        )
        ...
        ttk.Button(
            ...
            command=lambda: self.open_system_tool("control"),
            width=20,
        ).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(
            tools_frame,
            text="🔧 Services",
            command=lambda: self.open_system_tool("services.msc"),
            width=20,
        ).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(
            tools_frame,
            text="🖥️ MSConfig",
            command=lambda: self.open_system_tool("msconfig.exe"),
        ).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(
            tools_frame,
            text="📁 Task Manager",
            command=lambda: self.open_system_tool("taskmgr"),
            width=20,
        ).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(
            tools_frame,
            text="🔍 Programs & Features",
            command=lambda: self.open_system_tool("appwiz.cpl"),
            width=20,
        ).pack(fill=tk.X, pady=(0, 10))
        ttk.Button(
            tools_frame,
            text="🌐 Network Settings",
            command=lambda: self.open_system_tool("ncpa.cpl"),
            width=20,
        ).pack(fill=tk.X, pady=(0, 10))
    ...
    def _create_presets_tab(self):
        ...
        Layout:
        - Header with "Create Custom Preset" button.
        - Help section explaining how custom presets work.
        - Standard Presets grid (Gaming, Developer, Privacy, Performance, Battery).
        - Custom Presets grid (user-created .preset.json files).
        Each preset is rendered as a card with title, description, and Apply button.
        """
        # ── Scrollable container ─────────────────────────────────────────
        ...
        self.presets_window_id = self.presets_canvas.create_window(
            (0, 0), window=self.presets_scrollable, anchor="nw"
        )
        self.presets_canvas.configure(yscrollcommand=presets_scroll.set)
        self.presets_canvas.bind(
            "<Configure>",
            lambda e: self.presets_canvas.itemconfigure(
                self.presets_window_id, width=e.width
            ),
        )
        ...
        ttk.Label(container, text="Configuration Presets", style="Header.TLabel").pack(
            anchor="w", pady=(0, 20)
        )
        ...
        standard_presets.sort(
            key=lambda x: (
                built_in_order.index(x["id"]) if x["id"] in built_in_order else 99
            )
        )
        ...
        def render_presets(presets_list, parent_frame):
            for i, preset in enumerate(presets_list):
                ...
                ttk.Button(
                    btn_frame,
                    text="Apply Preset",
                    command=lambda p=preset["id"]: self.apply_preset(p),
                ).pack(side=tk.LEFT)
    ...
    def _create_manual_tab(self):
        """Build the Manual Configuration tab: search bar + scrollable setting rows.

        Layout:
        - Header with title and search entry field.
        - Scrollable canvas containing expandable setting rows.
        Settings are loaded from settings.json via SettingLoader and grouped by
        category.  Each row shows the setting name, current value, and an
        expand/collapse triangle that reveals the control options.
        """
        ...
        ttk.Label(
            header_frame, text="Manual Configuration", style="Header.TLabel"
        ).pack(side=tk.LEFT)
        ...
        self.manual_scrollable.bind(
            "<Configure>",
            lambda e: self.manual_canvas.configure(
                scrollregion=self.manual_canvas.bbox("all")
            ),
        )
        self.manual_window_id = self.manual_canvas.create_window(
            (0, 0), window=self.manual_scrollable, anchor="nw"
        )
        ...
        self.manual_canvas.bind(
            "<Configure>",
            lambda e: self.manual_canvas.itemconfigure(
                self.manual_window_id, width=e.width
            ),
        )
    ...
    def _create_history_tab(self):
        ...
        - Treeview showing columns: Timestamp, Setting, Old Value, New Value.
        - Footer with "Revert Selected Change" button.
        Data is sourced from the HistoryManager SQLite database.
        """
        ...
        header_frame = ttk.Frame(container)
        header_frame.pack(fill=tk.X, pady=(0, 20))
        ttk.Label(header_frame, text="Change History", style="Header.TLabel").pack(
            side=tk.LEFT
        )
        ttk.Button(
            header_frame, text="🔄 Refresh", command=self._refresh_history_tab
        ).pack(side=tk.RIGHT)
        ...
        ttk.Button(
            footer_frame,
            text="↩️ Revert Selected Change",
            command=self._revert_selected_change,
            style="Accent.TButton",
        ).pack(side=tk.LEFT)
        ttk.Label(
            footer_frame,
            text="Note: History logging is currently active for the Manual Configuration tab only.",
            style="Description.TLabel",
        ).pack(side=tk.RIGHT)
    ...
    def _refresh_history_tab(self):
        ...
        Fetches up to 100 recent changes from HistoryManager and inserts
        them into the treeview with alternating row colours.
        """
        ...
        for count, item in enumerate(history_data):
            ...
            self.history_tree.insert(
                "", "end", iid=item[0], values=item[1:], tags=(tag,)
            )

    def _revert_selected_change(self):
        """Revert the currently selected change in the history treeview.

        Reads the change details (hive, key_path, value_name, value_type, old_value)
        from HistoryManager, converts the old value back to its native type, and
        writes it to the Registry via RegistryHandler.
        """
        if not hasattr(self, "history_tree"):
            return
        ...
        if not selected_items:
            messagebox.showwarning(
                "No Selection", "Please select a change from the list to revert."
            )
            return
        ...
        if not details:
            messagebox.showerror(
                "Error", "Could not retrieve change details to perform revert."
            )
            return
        ...
        if messagebox.askyesno(
            "Confirm Revert",
            f"Are you sure you want to revert the change to '{value_name}'?",
        ):
            ...
            if self.registry_handler.write_value(
                hive, key_path, value_name, value_type, final_val
            ):
                ...
                messagebox.showerror(
                    "Error",
                    f"Failed to write to the registry to revert '{value_name}'.",
                )

    def _bind_home_scroll_events(self):
        """Rebind mouse-wheel scrolling for the Home tab canvas.

        The Home tab uses a Canvas + inner Frame pattern for scrolling.
        This method ensures the wheel event is captured on the canvas,
        the outer frame, and the scrollable inner frame so scrolling
        works regardless of where the cursor is within the tab.
        """
    ...
    def _bind_presets_scroll_events(self):
        """Rebind mouse-wheel scrolling for the Presets tab canvas.

        Guards each bind with winfo_exists() because the widgets may be
        destroyed during a tab rebuild or application shutdown.
        """
    ...
    def _bind_scroll_events(self):
        """Bind mouse-wheel scrolling for the Manual tab and all sub-tabs.

        Called once during _create_manual_tab and again whenever settings
        are refreshed (via _rebind_scroll_events).  Also ensures the
        notebook and main_frame forward wheel events to the active tab.
        """
        # Manual tab: bind canvas, frame, and scrollable inner frame
        ...
        # Delegate Home and Presets tab bindings to their dedicated methods
        self._bind_home_scroll_events()
        self._bind_presets_scroll_events()

        # Global bindings: notebook and main frame forward wheel to active tab
        self.notebook.bind("<MouseWheel>", self._on_global_mouse_wheel)
        self.main_frame.bind("<MouseWheel>", self._on_global_mouse_wheel)
        # Rebind when the manual canvas is mapped (shown) to capture new children
        self.manual_canvas.bind("<Map>", self._rebind_scroll_events)

    def _rebind_scroll_events(self, event=None):
        """Recursively rebind mouse-wheel events after widgets are added/removed.

        Called via the <Map> event on the manual canvas and after
        refresh_manual_config rebuilds the setting rows.  Ensures that
        newly created widgets (e.g. expanded option frames) also respond
    ...
    def _on_global_mouse_wheel(self, event):
        """Route mouse-wheel events to the currently active tab.

        Determines which tab is selected and delegates to the appropriate
        per-tab scroll handler.  This ensures scrolling works even when
        the cursor is over the notebook tabs or main frame margins.
        """
    ...
    def _on_mouse_wheel(self, event):
        ...
        The factor of 120 is the standard Windows wheel-delta unit;
        negating it makes positive delta scroll down (natural scrolling).
        """
    ...
    def _on_home_mouse_wheel(self, event):
        """Scroll the Home tab canvas in response to mouse wheel."""
    ...
    def _on_presets_mouse_wheel(self, event):
        """Scroll the Presets tab canvas in response to mouse wheel."""
        if self.presets_canvas and self.presets_canvas.winfo_exists():
            self.presets_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

    def refresh_manual_config(self):
        """Rebuild all setting rows in the Manual Configuration tab.

        Called when:
        - The user switches to the Manual tab.
        - The search query changes (via _on_search).

        Steps:
        1. Destroy all existing widgets in the scrollable frame.
        2. Read the current search query (case-insensitive).
        3. Iterate over categories and their settings, filtering by search.
        4. For each matching setting, create an expandable _create_setting_row.
        5. Rebind scroll events so new widgets respond to the mouse wheel.
        """
        print(
            f"DEBUG: refresh_manual_config called, categories: {len(self.setting_loader.get_categories())}"
        )
        # Clear existing setting rows before rebuilding
        ...
        for category in self.setting_loader.get_categories():
            ...
            filtered = [
                ...
                or search_query in (s.description or "").lower()
            ]

            if not filtered:
                continue
            print(
                f"DEBUG: Adding category {category.name} with {len(filtered)} settings"
            )
            ...
            ttk.Separator(category_container, orient="horizontal").pack(
                fill=tk.X, pady=(6, 0)
            )
    ...
    def _create_setting_row(self, parent, setting, is_alt=False):
        ...
        - A clickable header with an expand/collapse triangle, the setting name,
          and the current registry value displayed in user-friendly form.
        - A collapsible options panel (hidden by default) that contains the
          appropriate control widget: slider, text entry, button group, or
          simple enable/disable buttons.

        The row is stored in self.manual_row_widgets[setting_id] so that
        _update_row_selection_state can refresh it after a value change.

        Args:
            parent: The tk.Frame to pack this row into.
            setting: A RegistrySetting model object with hive, key_path, etc.
            is_alt: Unused; reserved for alternating row backgrounds.
        """
        bg = self.bg_color
        ...
        for widget in [triangle_label, name_label]:
            widget.bind(
                "<Button-1>",
                lambda e, sid=setting_id: self._toggle_setting_expansion(sid),
            )
        ...
        options_frame.pack(
            side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(34, 0), pady=(12, 0)
        )
        ...
        if self._should_use_slider(setting, options):
            ...
            buttons = self._create_text_control(
                options_frame, setting, current_val, options, setting_id
            )
        elif len(options) > 2:
            buttons = self._create_button_controls(
                options_frame, setting, current_val, options, setting_id
            )
        else:
            buttons = self._create_simple_controls(
                options_frame, setting, current_val, options, setting_id
            )
        ...
        if self._should_show_system_settings_link(setting):
            ...
            self._create_tooltip(
                link_btn, f"Open Windows Settings to manage {setting.name}"
            )
    ...
    def _should_use_slider(self, setting, options):
        ...
        if any(
            term in setting_name_lower for term in ["mousesensitivity", "mousespeed"]
        ):
            return True
        if (
            "mouse speed" in setting_display_name_lower
            or "mouse sensitivity" in setting_display_name_lower
        ):
            return True

        # Double-click speed
        if (
            "doubleclickspeed" in setting_name_lower
            or "double-click speed" in setting_display_name_lower
        ):
            return True
    ...
    def _create_slider_control(
        ...
        ttk.Label(control_frame, text="Adjust:", style="Description.TLabel").pack(
        ...
        if m:
            ...
            if (
                "mousesensitivity" in setting_name_lower
                or "mouse sensitivity" in setting_display_name_lower
            ):
                from_val, to_val = 1, 20
            elif (
                "mousespeed" in setting_name_lower
                or "mouse speed" in setting_display_name_lower
            ):
        ...
        try:
            current_num = (
                float(current_val) if current_val is not None else float(from_val)
            )
        ...
        value_label = ttk.Label(
            control_frame,
            text=f"Current: {int(current_num)}",
            style="Description.TLabel",
        )
        ...
        if (
            ...
            and to_val == 20
            and (
                "mousesensitivity" in str(setting.value_name).lower()
                or "enhance pointer precision" in setting_display_name_lower
            )
        ):
            ttk.Label(
                control_frame, text="(Low ← → High)", style="Description.TLabel"
            ).pack(side=tk.LEFT, padx=(5, 0))
        elif (
            from_val == 0
            and to_val == 2
            and "mouse speed" in setting_display_name_lower
        ):
            ttk.Label(
                control_frame, text="(Slow ← → Fast)", style="Description.TLabel"
            ).pack(side=tk.LEFT, padx=(5, 0))
            ...
            ttk.Label(
                control_frame, text="(Left ← → Center)", style="Description.TLabel"
            ).pack(side=tk.LEFT, padx=(5, 0))
        ...
        if callback:
            slider.bind(
                "<ButtonRelease-1>",
                lambda e, s=setting, v=slider_var: callback(s, v.get()),
            )
        else:
            slider.bind(
                "<ButtonRelease-1>",
                lambda e, sid=setting_id, s=setting, v=slider_var: (
                    self._apply_slider_value(sid, s, v)
                ),
            )
    ...
    def _parse_visual_effects_binary(self, binary_data):
        ...
        try:
            ...
            controls_animated = (
                byte_3 & 0x04
            ) == 0  # Bit 2 controls control animations
    ...
    def _resolve_system_settings_target(self, setting) -> str:
        ...
        if (
            "\\control panel\\mouse" in key_path
            or "\\control panel\\cursors" in key_path
        ):
        ...
        if (
            "\\accessibility\\stickykeys" in key_path
            or "\\accessibility\\keyboard response" in key_path
            or "\\accessibility\\togglekeys" in key_path
        ):
        ...
        if (
            "\\accessibility\\caption" in key_path
            or "\\accessibility\\configuration" in key_path
        ):
        ...
        if "power" in name_lower and "plan" in name_lower:
            return "uri:ms-settings:powersleep"
        if (
            "hibernate" in name_lower
            or "wake timer" in name_lower
            or "lid close" in name_lower
        ):
            return "cmd:rundll32.exe shell32.dll,Control_RunDLL powercfg.cpl,,3"
    ...
    def _create_button_controls(
        ...
        ttk.Label(control_frame, text="Select:", style="Description.TLabel").pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ...
        for option_name, option_value in options.items():
            ...
            if callback:
                option_btn.bind(
                    "<Button-1>",
                    lambda e, s=setting, v=option_value, btn=option_btn: callback(
                        s, v, btn
                    ),
                )
            else:
                option_btn.bind(
                    "<Button-1>",
                    lambda e, sid=setting_id, s=setting, v=option_value: (
                        self._apply_setting_value(sid, s, v)
                    ),
                )
    ...
    def _create_text_control(
        ...
        ttk.Label(control_frame, text="Value:", style="Description.TLabel").pack(
            side=tk.LEFT, padx=(0, 10)
        )
    ...
    def _collapse_setting_row(self, setting_id):
        """Hide the options panel for a setting and update the triangle to ▶."""
    ...
    def _expand_setting_row(self, setting_id):
        """Show the options panel for a setting and update the triangle to ▼."""
        row = self.manual_row_widgets.get(setting_id)
        if not row:
            return
        row["triangle"].config(text="▼")
    ...
    def _apply_and_log_change(self, setting, new_value):
        """Apply a setting change: write to Registry, log to history, and flag restart if needed.

        Steps:
        1. Read the current registry value (for history logging).
        2. Convert new_value to the correct Python type for the registry type.
        3. Write the value via RegistryHandler.
        4. On success, log the change to HistoryManager and mark restart_pending
           if the setting requires it.
        5. On failure, show an error dialog.

        Args:
            setting: The RegistrySetting being modified.
            new_value: The new value to write (may be str, int, etc.).

        Returns:
            True if the write succeeded, False otherwise.
        """
        old_value = self.registry_handler.read_value(
            setting.hive, setting.key_path, setting.value_name
        )

        # Convert new_value to the correct type for the registry
    ...
    def _apply_slider_value(self, setting_id, setting, var):
        """Apply a slider value to a setting and refresh the row's visual state.

        Called when the user releases the mouse button on a slider control.
        Rounds the float value to the nearest integer before applying.
        """
    ...
    def _apply_setting_value(self, setting_id, setting, value):
        """Apply a button/text value to a setting and refresh the row's visual state.

        Called by button controls (Enable/Disable, multi-option) and text entry.
        """
    ...
    def _update_row_selection_state(self, setting_id):
        """Refresh the visual state of a setting row after a value change.

        Re-reads the registry value and updates:
        - The "Current: ..." label with a user-friendly description.
        - Button styles (Accent.TButton for the selected option).
        - Slider position (if a slider control exists).
        """
        row = self.manual_row_widgets.get(setting_id)
        if not row:
            return
    ...
    def update_status(self, text, progress=None):
        """Update the status bar text and optionally show a progress bar.

        Args:
            text: Status message to display (e.g. "Applying settings...").
            progress: Optional float 0.0-1.0 representing completion percentage.
                      When provided, the progress bar is shown and updated.
                      When None, the progress bar is hidden.
        """
    ...
    def run_async(self, func, *args, **kwargs):
        """Execute a function in a background thread to keep the UI responsive.

        Sets is_busy=True to prevent concurrent operations.  When the task
        completes, resets is_busy, updates the status to "Ready", and
        prompts for a restart if any applied setting requires it.
    ...
    def _apply_settings_list(self, settings_list: List[Setting], profile_name: str):
        """Apply a list of setting objects to the system with restore point backup.

        Creates a temporary Profile from the list, checks for restart requirements,
        creates a system restore point, and applies all settings in safe_mode=False
        (including those that require a restart).

        Args:
            settings_list: List of Setting objects to apply.
            profile_name: Human-readable name for the restore point and status messages.
        """
        from src.models.profile import Profile

        # Build a temporary profile from the settings list
        ...
        def task():
            ...
            self.backup_manager.create_restore_point(
                f"WinSet Before Apply: {profile_name}"
            )
            ...
            messagebox.showinfo(
                "Success", f"Applied {sum(1 for v in results.values() if v)} settings."
            )

        self.run_async(task)

    def _save_settings_list_to_file(self, settings_list: List[Setting]):
        """Show a save dialog and export a list of settings to a JSON file.

        Used by the preset creation wizard to save user-configured settings
        as a reusable .json preset file.

        Args:
            settings_list: List of Setting objects with their values already set.
        """
    ...
    def _prompt_for_restart(self):
        """Show a dialog recommending a system restart after settings that require one.

        If the user accepts, triggers an immediate restart via `shutdown /r /t 1`.
        If the restart command fails, shows an error with manual restart instructions.
        """
        ...
        if messagebox.askyesno("Restart Recommended", msg):
            try:
                ...
                messagebox.showerror(
                    "Restart Failed",
                    f"Could not restart the system automatically.\nPlease restart manually.\n\nError: {e}",
                )

    def export_settings(self):
        """Export all current system settings to a JSON profile file.

        Reads every setting from settings.json, queries the current registry
        value for each, and writes them to a user-chosen file via ProfileExporter.
        ...
        try:
            ...
            def task():
                ...
                for i, cat in enumerate(all_cats):
                    self.update_status(
                        f"Reading {cat.value}...", 0.1 + (0.7 * (i / len(all_cats)))
                    )
                    category_settings = self.setting_loader.get_settings_for_category(
                        cat
                    )

                    for s in category_settings:
                        val = self.registry_handler.read_value(
                            s.hive, s.key_path, s.value_name
                        )
                        if val is not None:
                            s.value = val
                            settings.append(s)
                ...
                if self.exporter.export_profile(settings, file_path, "WinSet Export"):
                    ...
                    messagebox.showinfo(
                        "Success", f"Profile saved to {os.path.basename(file_path)}"
                    )
                else:
    ...
    def create_custom_preset(self):
        """Launch the two-step wizard to create a custom preset.

        Step 1 (_show_preset_selection_step): Let the user choose which settings
        to include from all available categories.
        Step 2 (_show_preset_configuration_step): Let the user set desired values
        for each selected setting, then save to file or apply immediately.
        """
        self._show_preset_selection_step()

    def _show_preset_selection_step(self):
        """Step 1 of the preset wizard: select settings to include.

        Displays a scrollable list of checkboxes grouped by category.
        The user selects one or more settings and clicks "Next" to proceed
        ...
        ttk.Label(header, text="Select Settings", style="Header.TLabel").pack(
            anchor="w"
        )
        ttk.Label(
            header,
            text="Choose which settings you want to include in your custom preset.",
            style="Description.TLabel",
        ).pack(anchor="w")
        ...
        ttk.Button(
            footer, text="Next >", style="Accent.TButton", command=next_step
    ...
    def _show_preset_configuration_step(self, selected_settings):
        ...
        header = ttk.Frame(dialog, padding=20)
        header.pack(fill=tk.X)
        ttk.Label(header, text="Configure Values", style="Header.TLabel").pack(
            anchor="w"
        )
        ttk.Label(
            header,
            text="Set the desired values for your preset.",
            style="Description.TLabel",
        ).pack(anchor="w")
        ...
        def apply_settings():
            ...
            if messagebox.askyesno(
                "Save Preset?",
                "Settings applied. Would you also like to save these settings as a preset file for later use?",
            ):
                self._save_settings_list_to_file(final_settings)
        ...
        ttk.Button(
            footer,
            text="Back",
            command=lambda: [dialog.destroy(), self._show_preset_selection_step()],
        ).pack(side=tk.LEFT)
        ttk.Button(footer, text="Cancel", command=dialog.destroy).pack(
            side=tk.RIGHT, padx=5
        )
        ttk.Button(footer, text="💾 Save to File...", command=save_to_file).pack(
            side=tk.RIGHT, padx=5
        )
        ttk.Button(
            footer,
    ...
    def center_dialog(self, dialog):
        """Centre a Toplevel dialog over the main application window.

        Forces Tk to compute the dialog's actual size before calculating
        the offset so the dialog is perfectly centred.
        """
    ...
    def import_settings(self):
        """Import settings from a previously exported JSON profile file.

        Opens a file picker, loads the profile via ProfileImporter, and
        shows a selection dialog so the user can choose which settings
        to apply from the profile.
        """
    ...
    def _show_import_selection_dialog(self, profile):
        """Show a dialog to select specific settings from an imported profile.

        Displays all settings from the profile grouped by category, each
        with a checkbox (checked by default).  The user can select/deselect
        individual settings or toggle all, then click "Confirm" to apply
        the selected settings with a system restore point backup.

        Args:
            profile: A Profile object loaded by ProfileImporter.
        """
        dialog = tk.Toplevel(self.root)
        ...
        header.pack(fill=tk.X)
        ttk.Label(header, text="Select Settings to Import", style="Header.TLabel").pack(
            anchor="w"
        )
        ttk.Label(
            header, text=f"Profile: {profile.name}", style="Description.TLabel"
        ).pack(anchor="w")
        ...
        for cat, settings in settings_by_cat.items():
            ...
            ttk.Label(
                cat_frame, text=cat.replace("_", " ").title(), style="Bold.TLabel"
        ...
        def apply():
            ...
            if messagebox.askyesno(
                "Confirm",
                f"Apply {len(selected_settings)} settings from '{profile.name}'?",
            ):

                def task():
                    self.update_status("Creating Restore Point...", 0.2)
                    self.backup_manager.create_restore_point(
                        f"WinSet Import: {profile.name}"
                    )
                    ...
                    messagebox.showinfo(
                        "Success",
                        f"Applied {sum(1 for v in results.values() if v)} settings.",
                    )
        ...
        ttk.Button(footer, text="Confirm", style="Accent.TButton", command=apply).pack(
            side=tk.RIGHT, padx=5
        )
        ...
        ttk.Button(footer, text="Select All", command=lambda: toggle_all(True)).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(footer, text="Deselect All", command=lambda: toggle_all(False)).pack(
            side=tk.LEFT, padx=5
        )

    def apply_preset(self, preset_id):
        """Apply a built-in or custom preset after user confirmation.

        Shows a confirmation dialog explaining that presets act as overlays
        (only modifying the settings they contain).  If confirmed, creates
        a system restore point, then applies the preset in a background thread.

        Args:
            preset_id: The unique identifier of the preset (e.g. "gaming").
        """
        msg = (
        ...
        if messagebox.askyesno("Confirm Preset Application", msg):
            ...
            def task():
                ...
                self.backup_manager.create_restore_point(
                    f"WinSet Before Preset: {preset_id}"
                )
    ...
    def create_restore_point(self):
        """Create a manual system restore point via BackupManager.

        Runs in a background thread and shows a success/error dialog when complete.
        """

        def task():
            self.update_status("Creating Restore Point...", 0.5)
            success = self.backup_manager.create_restore_point(
                "WinSet Manual Restore Point"
            )
            if success:
    ...
    def open_system_tool(self, tool_name):
        """Launch a Windows system tool or Control Panel applet by name.

        Used by the Home tab buttons to open tools like control.exe,
        services.msc, msconfig.exe, taskmgr, etc.
